...
const std::string userName = "thomasmartens";
const std::string password = "HRThomas";
const std::string profileName = "thomas";

unsigned int userID = 0;
int userCloudID = -1;
bool enableLoger = false;

int version	= -1; // laatste versie
bool running = true;
...
static bool handleUserInput()
{
	...
	case '1':
	{
		...
		ulong action1 = (ulong)IEE_MentalCommandAction_t::MC_LIFT;
		ulong action2 = (ulong)IEE_MentalCommandAction_t::MC_DROP;
		ulong listAction = action1 | action2;
		int errorCode = EDK_OK;
		errorCode = IEE_MentalCommandSetActiveActions(userID, listAction);
		if (errorCode == EDK_OK)
		{
			std::cout << "Setting MentalCommand active actions (MC_LIFT | MC_DROP) for user " << userID << std::endl;			
		}
	...
	case '2':
		...
		break;
	case '3':
		std::cout << std::endl << "Starting training DROP !" << std::endl;
		int errorCode;
		errorCode = IEE_MentalCommandSetTrainingAction(userID, MC_DROP);
		errorCode = IEE_MentalCommandSetTrainingControl(userID, MC_START);
		break;
	case '4':
		std::cout << std::endl << "Starting training LIFT !" << std::endl;
		IEE_MentalCommandSetTrainingAction(userID, MC_LIFT);
		IEE_MentalCommandSetTrainingControl(userID, MC_START);
		break;
	case '5':
	{
...
int main(int argc, char** argv)
{
	...
	std::cout << "===================================================================" << std::endl;
    std::cout << "Software die verbindt met Emotiv Server, profiel laad/saved en vervolgens kan je trainingen. " << std::endl;
    std::cout << "===================================================================" << std::endl;
...
void help()
{
    ...
    std::cout << "B. Press 2 to Start training NEUTRAL." << std::endl;
    std::cout << "C. Press 3 to Start training DROP." << std::endl;
    std::cout << "D. Press 4 to Start training LIFT." << std::endl;
